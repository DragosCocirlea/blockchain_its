...
// makeMasterHost creates a LibP2P host with a random peer ID listening on the given multiaddress
func makeMasterHost(listenPort int, randseed int64) (host.Host, error) {
	...
	fmt.Printf("I am master node %s\n", basicHost.ID().Pretty())
	fmt.Printf("\nNow run this on a different terminal in the region directory:\ngo run *.go -port %d -peer %s\n\n", listenPort+1, fullAddr)
